(function($) {
  ...
  function toggleSidebar() {
    ...
    if (navItem.length) {
      ...
      var offsetTop = $('.sidebar-section').position().top + $('.sidebar-section-nav__item.is-active').position().top;
      el.next().css('top', offsetTop);
    }
  ...
  $(document).on('click', function (e) {
    ...
    if ($('body').hasClass('sidebar-sm') || $('body').hasClass('sidebar-md')) {
      var isSidebarToggleCheckbox = $(e.target).closest('.switch').find('#collapse-sidebar').length;
      ...
      if (!$(e.target).closest('.sidebar-section').length && !isSidebarToggleCheckbox) {
        $('.sidebar-section-subnav').hide();
        $('.sidebar-section-nav__item').removeClass('is-active');
      }
    }
